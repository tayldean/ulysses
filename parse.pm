...
# 
# The total number of entrants per event is not being counted if the
# school didn't get points.
        ...
        foreach $school (sort {$skater_school_hash{$a} <=> $skater_school_hash{$b}} keys %skater_school_hash){
            foreach $skater (sort {$skater_point_hash{$b} <=> $skater_point_hash{$a}} keys %skater_point_hash){
                if ($skater_school_hash{$skater} eq $school) {
                    print FILE_OUT "  ",$skater,",",$skater_school_hash{$skater},",",$skater_point_hash{$skater},",",$skater_start_hash{$skater},",",sprintf("%0.2f",$skater_point_hash{$skater}/$skater_start_hash{$skater}),"\n";
                }
            }
        }

        print FILE_OUT "---------------------------------------- Team Points & Starts\n";
        print FILE_OUT "School,Points,Starts,Points/Start\n";
        foreach $school (sort {$total_hash{$b} <=> $total_hash{$a}} keys %total_hash){
            print FILE_OUT $school,",",$total_hash{$school},",",$total_starts_hash{$school},",",sprintf("%0.2f",$total_hash{$school}/$total_starts_hash{$school}),"\n";
